...
export function MarkerNode(props) { 
  ...
  return div(
    ...
    [
      ...
      div('.node-outputs', [
        span('.output-point.bool-data', { dataset: { type: 'output', name: 'present', parent: uuid, offsetX: '280', offsetY: '40' }}, 'DETECT'),
        span('.output-point.number-data', { dataset: { type: 'output', name: 'posX', parent: uuid, offsetX: '280', offsetY: '60' }}, 'X'),
        span('.output-point.number-data', { dataset: { type: 'output', name: 'posY', parent: uuid, offsetX: '280', offsetY: '80' }}, 'Y'),
        span('.output-point.number-data', { dataset: { type: 'output', name: 'rotation', parent: uuid, offsetX: '280', offsetY: '100' }}, 'ANGLE'),
      ])
    ]
  );
};
...
export function KeyPressNode(props) {
  ...
  return div(
    '.draggable-node.dark-node.key-node',
    { style: { transform: `translate(${x}px, ${y}px)` }, class: { selected },  dataset: { uuid } },
    [
      div('.node-inputs', [
